...
export default function NotificationBell() {
  ...
  const generateSystemNotifications = useCallback(async (): Promise<Notification[]> => {
    ...
    try {
      ...
      const lowStockRes = await fetch(`${API_BASE}/api/products?lowStock=true&limit=20&sort=-updatedAt`, {
        headers: getAuthHeaders(),
      });
      if (lowStockRes.ok) {
        const lowStockData = await lowStockRes.json();
        const lowStockProducts = lowStockData.data || lowStockData.products || [];
        lowStockProducts.slice(0, 8).forEach((product: { _id: string; name: string; stockQuantity?: number; stock?: number }, index: number) => {
    ...
    try {
      ...
      const receiptsRes = await fetch(`${API_BASE}/api/receipts?limit=5&sort=-createdAt`, {
        headers: getAuthHeaders(),
      });
      if (receiptsRes.ok) {
        ...
        receipts.forEach((receipt: { _id: string; receiptNumber?: string; supplier?: string; status?: string; createdAt?: string }, index: number) => {
          if (["draft", "waiting", "ready", "pending"].includes(String(receipt.status || "").toLowerCase())) {
            generatedNotifications.push({
    ...
    try {
      ...
      const deliveryRes = await fetch(`${API_BASE}/api/delivery-orders?limit=8`, {
        headers: getAuthHeaders(),
      });
      if (deliveryRes.ok) {
        ...
        deliveries.forEach((delivery: { _id: string; orderNumber?: string; orderId?: string; customer?: string; createdAt?: string }, index: number) => {
          const status = String((delivery as { status?: string }).status || "").toLowerCase();
          if (["delivered", "cancelled", "done"].includes(status)) return;
          generatedNotifications.push({
  ...
  useEffect(() => {
    ...
    const events = ["notification:new", "product_update", "delivery_update", "transfer_update", "dashboard:refresh"];
    events.forEach((eventName) => on(eventName, handler));
    return () => events.forEach((eventName) => off(eventName, handler));
