...
function TransferModal({
  ...
  const availableProducts = formData.fromWarehouseId
    ? products.filter((product) => product.warehouseId === formData.fromWarehouseId)
    : products;
  ...
  const handleSubmit = async () => {
    ...
    if (formData.fromWarehouseId === formData.toWarehouseId && formData.mode !== "rack") {
      setFormData((current) => ({ ...current, mode: "rack" }));
    }
    ...
    for (const line of validItems) {
      ...
      if (formData.fromWarehouseId && product.warehouseId && product.warehouseId !== formData.fromWarehouseId) {
        setError(`Product ${product.name} does not belong to selected source warehouse.`);
        return;
      }
      const available = getAvailableStock(line.productId);
      if (available > 0 && line.quantity > available) {
        setError(`Quantity for ${product.name} exceeds available stock (${available}).`);
        return;
      }
    }
                        ...
                        <div key={`item-row-${index}`} className="grid grid-cols-1 md:grid-cols-[1.2fr_0.7fr_auto] gap-3">
                          <select value={item.productId} onChange={(e) => updateItem(index, "productId", e.target.value)} className="px-4 py-3 bg-[var(--card-bg)] border border-[var(--glass-border)] rounded-xl text-[var(--text-primary)] focus:border-indigo-300 focus:ring-4 focus:ring-indigo-500/10 focus:outline-none">
                            ...
                            {availableProducts.map((product) => (
                              <option key={product.id} value={product.id}>{product.name} ({product.sku}){typeof product.stockQuantity === "number" ? ` • stock ${product.stockQuantity}` : ""}</option>
                            ))}
                          </select>
                          <input
                            value={item.quantity}
                            ...
                            step="1"
                            min="1"
                            max={Math.max(1, getAvailableStock(item.productId) || 1)}
                            disabled={!item.productId}
                            placeholder={item.productId ? `Max ${getAvailableStock(item.productId) || 0}` : "Select product first"}
                            className="px-4 py-3 bg-[var(--card-bg)] border border-[var(--glass-border)] rounded-xl text-[var(--text-primary)] placeholder-slate-400 focus:border-indigo-300 focus:ring-4 focus:ring-indigo-500/10 focus:outline-none disabled:opacity-60"
                          />
  ...
  const fetchReferenceData = useCallback(async () => {
    try {
      const [warehousesRes, productsRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/api/products?limit=10000`, { headers: getAuthHeaders() }),
      ]);
      ...
      if (productsRes.ok) {
        ...
        const mappedProducts = ((productData.data || productData) as Array<{ _id: string; name: string; sku: string; warehouse?: { _id?: string; name?: string } | string; stockQuantity?: number }>).map((product) => ({
          id: product._id,
          name: product.name,
          sku: product.sku,
          warehouseId: typeof product.warehouse === "string" ? product.warehouse : product.warehouse?._id,
          warehouseName: typeof product.warehouse === "string" ? "" : product.warehouse?.name,
