...
function normalizeProducts(rawProducts: Array<Partial<Omit<Product, "clientId">>>): Product[] {
  return rawProducts.map((product, index) => {
    const sourceId = typeof product._id === "string" ? product._id.trim() : "";
    const sku = typeof product.sku === "string" ? product.sku : "";
    const name = typeof product.name === "string" ? product.name : `Product ${index + 1}`;
    const identityBase = sourceId || sku || name || "product";

    return {
      clientId: `${identityBase}-${index}`,
      _id: sourceId,
      name,
      sku,
      price: product.price,
...
export default function BulkLabelPrinter({
  ...
  const selectAll = () => {
    if (selectedIds.size === filteredProducts.length) {
      ...
      setSelectedIds(new Set(filteredProducts.map(p => p.clientId)));
    }
  };
  ...
  const selectedProducts = products.filter(p => selectedIds.has(p.clientId));
                  ...
                  <div className="space-y-1">
                    {filteredProducts.map((product) => (
                      <motion.div
                        ...
                        onClick={() => toggleSelect(product.clientId)}
                        className={`p-3 rounded-xl cursor-pointer transition-all flex items-center gap-3 ${
                          selectedIds.has(product.clientId)
                      ...
                      >
                        <div className={`w-5 h-5 rounded flex items-center justify-center ${
                          ...
                          {selectedIds.has(product.clientId) && <Check className="w-3 h-3" />}
                        </div>
                ...
                {selectedProducts.length === 0 ? (
                  ...
                  >
                    {labelsToGenerate.slice(0, 20).map((item, idx) => (
                      <BarcodeLabel
                        key={`${item.product.clientId}-${item.copyIndex}`}
