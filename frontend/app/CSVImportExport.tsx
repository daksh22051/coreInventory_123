...
export default function CSVImportExport({
  ...
  const handleImport = async () => {
    ...
    const getSkuKey = (value: string) => (value || "").trim().toUpperCase();

    let skuToId = new Map<string, string>();
    try {
      const existingRes = await fetch(`${API_BASE}/api/products?limit=10000`, {
      ...
      skuToId = new Map(
        existingProducts
          .filter((p: Record<string, unknown>) => p?._id && p?.sku)
          .map((p: Record<string, unknown>) => [String(p.sku).trim().toUpperCase(), String(p._id)])
      );
    } catch {
      // If this prefetch fails, import still continues with create-first fallback.
    }

    for (const row of validRows) {
      try {
        const normalizedCategory = normalizeCategory(row.data.category || "");
        const sku = getSkuKey(row.data.sku);
        const payload = {
          name: row.data.name,
          sku,
        ...
        if (existingId) {
          const updateRes = await fetch(`${API_BASE}/api/products/${existingId}`, {
            method: "PUT",
            headers: getAuthHeaders(),
          ...
          result.failed++;
          result.errors.push({ row: row.rowNumber, message: updateData.message || "Failed to update existing SKU" });
          continue;
        }
        ...
        if (data.success || res.ok) {
          result.success++;
          if (data?.data?._id) {
            skuToId.set(sku, String(data.data._id));
          }
        } else {
          const isDuplicateSku = /duplicate key|E11000|sku/i.test(String(data?.message || ""));

          if (isDuplicateSku) {
            try {
              const findRes = await fetch(`${API_BASE}/api/products?search=${encodeURIComponent(sku)}&limit=5`, {
                headers: getAuthHeaders(),
              });
              const findData = await findRes.json();
              const matched = (findData?.data || []).find((p: Record<string, unknown>) => getSkuKey(String(p?.sku || "")) === sku);

              if (matched?._id) {
                const updateRes = await fetch(`${API_BASE}/api/products/${matched._id}`, {
                  method: "PUT",
                  headers: getAuthHeaders(),
                  body: JSON.stringify(payload),
                });
                ...
                if (updateData.success || updateRes.ok) {
                  result.success++;
                  skuToId.set(sku, String(matched._id));
                  continue;
                }
              }
            } catch {
          ...
          result.failed++;
          result.errors.push({ row: row.rowNumber, message: data.message || "Failed to import row" });
        }
      } catch (err) {
        result.failed++;
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (exportOptions.format === "csv") {
        ...
        const buildPdfBrandingAssets = async (verificationId: string) => {
          const logoCanvas = document.createElement("canvas");
          logoCanvas.width = 260;
          logoCanvas.height = 72;
          const ctx = logoCanvas.getContext("2d");

          if (ctx) {
            const gradient = ctx.createLinearGradient(0, 0, 260, 72);
            ...
            ctx.fillText("CI", 18, 46);
            ctx.font = "bold 16px Arial";
            ctx.fillText("CoreInventory", 70, 33);
            ctx.font = "12px Arial";
            ctx.fillStyle = "#bfdbfe";
            ctx.fillText("Inventory Intelligence", 70, 52);
          }

          const { default: QRCode } = await import("qrcode");
          const qrDataUrl = await QRCode.toDataURL(`https://coreinventory.local/verify/${verificationId}`, {
            width: 128,
            margin: 1,
        ...
        const now = new Date();
        const verificationId = `CI-${now.getTime().toString(36).toUpperCase()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`;
        const { logoDataUrl, qrDataUrl } = await buildPdfBrandingAssets(verificationId);

        const reportHeaders = ["Product Name", "SKU"];
        if (exportOptions.includeCategory) reportHeaders.push("Category");
        if (exportOptions.includePrice) reportHeaders.push("Price");
        if (exportOptions.includeStock) reportHeaders.push("Stock Qty");
        reportHeaders.push("Stock State");
        reportHeaders.push("Description");

        const tableRows = products.map((p: Record<string, unknown>) => {
          const stockQty = Number(p.stockQuantity || 0);
          const stockState = stockQty === 0 ? "OUT-OF-STOCK" : stockQty <= 10 ? "LOW-STOCK" : "IN-STOCK";
          const row: string[] = [
            String(p.name || "-"),
            String(p.sku || "-"),
          ];
          if (exportOptions.includeCategory) row.push(String(p.category || "General"));
          if (exportOptions.includePrice) row.push(`Rs. ${Number(p.price || 0).toLocaleString("en-IN")}`);
          if (exportOptions.includeStock) row.push(String(stockQty));
          row.push(stockState);
          row.push(String(p.description || "-").slice(0, 90));
          return row;
        });

        const totalStockValue = products.reduce((sum: number, p: Record<string, unknown>) => {
        ...
        doc.addImage(logoDataUrl, "PNG", 40, 12, 168, 46);

        doc.setTextColor(255, 255, 255);
        doc.setFont("helvetica", "bold");
        doc.setFontSize(20);
        doc.text("CoreInventory Premium Product Report", 226, 38);
        doc.setFont("helvetica", "normal");
        doc.setFontSize(10);
        doc.text(`Generated: ${now.toLocaleString("en-IN")}`, 226, 58);
        doc.text(`Format: PDF | Records: ${products.length}`, 226, 73);

        doc.setTextColor(191, 219, 254);
        doc.setFont("helvetica", "normal");
        doc.setFontSize(9);
        doc.text(`Verification ID: ${verificationId}`, pageWidth - 40, 78, { align: "right" });

        doc.setTextColor(44, 62, 80);
        doc.setFillColor(240, 244, 255);
        doc.roundedRect(40, 110, 220, 52, 8, 8, "F");
        doc.roundedRect(274, 110, 220, 52, 8, 8, "F");
        doc.roundedRect(508, 110, 220, 52, 8, 8, "F");

        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
        doc.text("TOTAL PRODUCTS", 52, 130);
        doc.text("INCLUDED FIELDS", 286, 130);
        doc.text("EST. STOCK VALUE", 520, 130);

        doc.setFontSize(16);
        doc.text(String(products.length), 52, 150);
        doc.text(String(reportHeaders.length), 286, 150);
        doc.text(`Rs. ${Math.round(totalStockValue).toLocaleString("en-IN")}`, 520, 150);

        const stockStateColumnIndex = reportHeaders.indexOf("Stock State");
        const tableStartY = 186;
        ...
        autoTable(doc, {
          ...
          didParseCell: (hook) => {
            if (hook.section === "body" && hook.column.index === stockStateColumnIndex) {
              const value = String(hook.cell.raw || "");
              hook.cell.styles.fontStyle = "bold";
              if (value.includes("IN-STOCK")) hook.cell.styles.textColor = [22, 163, 74];
              else if (value.includes("LOW-STOCK")) hook.cell.styles.textColor = [217, 119, 6];
              else hook.cell.styles.textColor = [220, 38, 38];
            }
          },
          willDrawPage: () => {
            ...
            doc.setFontSize(34);
            doc.setTextColor(242, 246, 252);
            doc.setFont("helvetica", "bold");
            doc.text("COREINVENTORY", pageWidth / 2, pageHeight / 2 + 40, { align: "center", angle: 25 });
          },
          didDrawPage: (data) => {
            ...
            doc.text("CoreInventory | Product Intelligence Export", 40, footerY);
            doc.setFont("times", "italic");
            doc.setTextColor(30, 64, 175);
            doc.text("Digitally Signed: CoreInventory Secure Seal", 280, footerY);
            doc.setFont("helvetica", "normal");
            doc.setTextColor(100, 116, 139);
            doc.text(`Page ${data.pageNumber}`, pageWidth - 86, footerY);
          },
        });

        const finalTableY = ((doc as unknown as { lastAutoTable?: { finalY?: number } }).lastAutoTable?.finalY ?? 182) + 20;
        const pageHeight = doc.internal.pageSize.getHeight();
        const signY = finalTableY > pageHeight - 115 ? 90 : finalTableY;

        if (finalTableY > pageHeight - 115) {
          doc.addPage();
        }

        doc.setDrawColor(226, 232, 240);
        doc.setFillColor(248, 250, 252);
        doc.roundedRect(40, signY, pageWidth - 80, 110, 8, 8, "FD");
        doc.setFont("helvetica", "bold");
        doc.setTextColor(51, 65, 85);
        doc.setFontSize(11);
        doc.text("Approval & Signature", 52, signY + 20);
        doc.setFont("helvetica", "normal");
        doc.setFontSize(9);
        doc.text("Prepared By", 52, signY + 56);
        doc.text("Verified By", 260, signY + 56);
        doc.text("Authorized Signatory", 468, signY + 56);
        doc.setFontSize(8);
        doc.setTextColor(71, 85, 105);
        doc.text(`Verification ID: ${verificationId}`, 52, signY + 74);
        doc.text(`Issued: ${now.toLocaleDateString("en-IN")}`, 260, signY + 74);
        doc.text("Signature Type: Digital", 468, signY + 74);
        doc.setFillColor(255, 255, 255);
        doc.roundedRect(pageWidth - 150, signY + 14, 88, 88, 8, 8, "F");
        doc.addImage(qrDataUrl, "PNG", pageWidth - 143, signY + 20, 74, 74);
        doc.setFontSize(8);
        doc.setTextColor(51, 65, 85);
        doc.text("Scan to verify", pageWidth - 106, signY + 106, { align: "center" });
        doc.line(52, signY + 48, 220, signY + 48);
        doc.line(260, signY + 48, 428, signY + 48);
        doc.line(468, signY + 48, pageWidth - 160, signY + 48);

        doc.save(`products_report_${now.toISOString().slice(0, 10)}.pdf`);
      }
    } catch (err) {
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      <option value="pdf">Premium PDF (.pdf)</option>
